...
script_path <- if (length(file_arg)) {
  ...
  "problem-sets/ps5/scripts/prepare-phoible.R"
}
ps5_dir <- normalizePath(file.path(dirname(script_path), ".."), mustWork = TRUE)
output_path <- file.path(ps5_dir, "data", "phoible-pilot.rds")
...
provided_archive <- Sys.getenv("PHOIBLE_ARCHIVE", unset = "")
archive_path <- if (nzchar(provided_archive)) {
  normalizePath(provided_archive, mustWork = TRUE)
} else {
...
renamings <- c(
  ...
  Latitude = "latitude",
  Longitude = "longitude",
  ISO639P3code = "iso639p3",
  Family_Glottocode = "family_glottocode",
  Family_Name = "family_name"
)
for (old_name in names(renamings)) {
  names(inventory_metadata)[names(inventory_metadata) == old_name] <-
    renamings[[old_name]]
}
...
cat("Retained consonant and vowel parameters:", nrow(segment_metadata), "\n")
