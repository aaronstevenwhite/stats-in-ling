...
To interpret the difference, we must specify the response, the linguistic contrast, the observations being compared, and the population quantity we want to estimate.
...
Any coefficient we estimate must be interpreted relative to this unit, response, comparison, and target. The data and design do not support a broader interpretation.
...
A dataset does not determine a single analysis. The research question determines which observations to compare, which response to analyze, and which population quantity to estimate.
...
None of these problems can be solved by changing the estimator. No model can recover a comparison that the data never contained.
...
In that case, revise the question and estimand before continuing. Exploration may change the analysis, but the reported result should be tied to the revised question rather than presented as an answer to the original one.
