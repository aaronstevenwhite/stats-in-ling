...
The [sample space](possibilities-and-events.qmd) $\Omega$ specifies which outcomes the model represents, and the [sigma-algebra](event-spaces.qmd) $\mathcal{F}$ specifies which events are measurable. Neither object assigns a probability to those events.
...
The notation states that $\mathbb{P}$ is a function whose domain is $\mathcal{F}$ and whose codomain is the closed interval $[0,1]$. It takes an event as its input and returns that event's probability.
...
The value $.70$ is the probability assigned to $A$ by this particular measure. It is not a property of accusative case independently of the sample space and probability measure.
...
Nonnegativity rules out negative probabilities. Normalization assigns unit probability to the certain event. Countable additivity makes the probability of a union of pairwise disjoint events equal to the sum of their individual probabilities.
...
This calculation applies countable additivity. It extends the assignment from singleton events to a larger event without introducing an additional corpus estimate.
...
An assignment that violates any of the three requirements is not a probability measure. Suppose the four pronoun values were $.20$, $.30$, $.10$, and $.50$. Although each value lies between zero and one, their sum is $1.10$, so the assignment violates normalization.
...
The [next page](probability-concepts.qmd) uses the probability measure to characterize events that cannot occur together.
