...
In the example above, the accusative event does not belong to $\mathcal{F}_P$. A model whose event space is $\mathcal{F}_P$ can assign probabilities to number but not to case. To ask about case, we must first include the accusative event in the event space.
