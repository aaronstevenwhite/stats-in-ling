...
The **distribution** of $X$ is the probability measure $\mathbb P_X$ on $\mathcal X$ defined by
...
A real-valued random variable $X$ is **absolutely continuous** when there is a nonnegative function $f_X$ such that, for every Borel set $A\subseteq\mathbb R$,
...
$$
\mathbb P(80\leq D\leq100)=\int_{80}^{100}\frac{1}{100}\,\mathrm d d=.20.
$$
...
\int_{-\infty}^{\infty}x f_X(x)\,dx,&X\text{ absolutely continuous}.
\end{cases}
$$
...
For fixed $p$, the events $\{P=p,R=r\}$ partition $\{P=p\}$ as $r$ ranges over the support of $R$. Countable additivity gives
...
$$
f_X(x)
\equiv\int_{-\infty}^{\infty}f_{X,Y}(x,y)\,dy.
$$
