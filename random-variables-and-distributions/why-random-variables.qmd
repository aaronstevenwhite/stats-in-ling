...
The foundations chapter defined [outcomes and sample spaces](../foundations/possibilities-and-events.qmd#outcome-granularity), [events](../foundations/events.qmd), and [probability measures](../foundations/probability-measures.qmd#the-three-requirements). Suppose one outcome in a phonetics study is a complete vowel token. The stored record includes the speaker, word, waveform, vowel label, and measured duration, but the research question may concern only duration. How can we ask a probability question about duration while retaining the complete token as the outcome?
...
The duration is an acoustic measurement attached to a token. It does not by itself determine the token's phonological category, speaker population, or lexical identity; those distinctions remain in the outcome record.
...
Basically, a [**random variable**](https://bruno.nicenboim.me/bayescogsci/ch-intro.html) maps outcomes to values. More specifically, probability statements about those values refer to the corresponding events in the original sample space. The [next page](defining-random-variables.qmd) makes this correspondence precise with preimages and measurability.
