...
A random variable whose distribution has a probability density function is [**absolutely continuous**](https://stats.libretexts.org/Bookshelves/Probability_Theory/Applied_Probability_%28Pfeiffer%29/07%3A_Distribution_and_Density_Functions/7.01%3A_Distribution_and_Density_Functions). Introductory statistics texts commonly shorten this to *continuous random variable*, and we will use that shorter wording after making the restriction explicit. Interval probabilities are obtained by integrating the density. Such a distribution assigns probability zero to every individual real value, even when its density is positive there.
...
The fact that a single point has zero probability follows from the [density representation](probability-density-functions.qmd). Probability is assigned across regions rather than as separate positive masses at each point.
