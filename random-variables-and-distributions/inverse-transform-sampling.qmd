...
The [cumulative distribution function](cumulative-distribution-functions.qmd) records the probability accumulated through each threshold. Suppose we can draw a uniform number between zero and one, but we want a boundary label with a nonuniform target distribution. How can cumulative thresholds convert the first draw into the second?

Suppose a prosodic annotator assigns one of three boundary strengths with probabilities
...
The labels and masses are constructed. Prosodic boundary labels are outputs of a transcription system, not direct acoustic quantities; for instance, [ToBI](https://www.isca-archive.org/icslp_1992/silverman92_icslp.html) represents tones and break indices on distinct annotation tiers. The sampling calculation reproduces a declared label distribution, not prosodic ground truth.
...
Basically, we divide the unit interval into pieces whose widths are the target probabilities. More specifically, [**inverse transform sampling**](https://bookdown.org/richard_g_everitt/notes/chapsimulation.html) uses a uniform draw on $(0,1)$ and the target CDF to make that conversion.
