...
The earlier [density page](probability-density-functions.qmd) represented continuous probability by area. Suppose a psycholinguistic experiment places an auditory cue at a randomized time between 200 and 800 ms after word onset, with equal width intervals inside this window equally probable. What density makes every equal-width interval equally likely?

The answer is a rectangle whose total area is one. More specifically, the [**continuous uniform distribution**](https://bruno.nicenboim.me/bayescogsci/ch-intro.html) represents a constant density across a bounded interval. Write
...
## The $\operatorname{Uniform}(0,1)$ density

First graph the unit-uniform density.

```{r}
x <- seq(-1, 2, length.out = 800)
plot(x, dunif(x, 0, 1), type = "l", lwd = 3, col = "#2166AC",
...
The shaded area in the next graph is this probability.
...
polygon(c(.25, .25, .75, .75), c(0, 1, 1, 0),
        col = adjustcolor("#B2182B", alpha.f = .45), border = NA)
lines(x, dunif(x, 0, 1), lwd = 3, col = "#2166AC")
```
...
```{r}
plot(x, punif(x, 0, 1), type = "l", lwd = 3, col = "#2166AC",
