...
The [preceding page](expectation-of-functions.qmd) showed that a transformation is ordinarily applied before its expectation is taken. A linear combination has an additional simplification. Suppose a corpus sample contains three clause positions, and $I_i$ records whether position $i$ contains a resumptive pronoun. Must we derive the complete distribution of the total before finding its expectation?

$$
T\equiv I_1+I_2+I_3.
$$

We want the expected total count. Deriving every possible value of $T$ would require tracking all combinations of the three indicators. Basically, we can average each contribution and then add those averages. More specifically, [**linearity of expectation**](https://stats.libretexts.org/Bookshelves/Probability_Theory/Probability_Mathematical_Statistics_and_Stochastic_Processes_%28Siegrist%29/04%3A_Expected_Value/4.01%3A_Definitions_and_Basic_Properties) lets us work term by term instead.
...
Dependence, as defined for [events in the foundations module](../foundations/independence.qmd), affects how probability is arranged across the possible total counts. It does not change the expected sum when the three marginal event probabilities remain fixed.
