...
The [Bernoulli distribution](bernoulli-distribution.qmd) represents one binary observation. Suppose, however, that the response is the number of target outcomes across a fixed set of positions. How do we move from one binary observation to that total? The binomial distribution answers this question under additional assumptions.

Suppose a bilingual production task gives a participant six positions at which a code switch could occur.
...
The six opportunities are hypothetical. In an empirical code-switching study, an eligible position must be defined from the bilingual discourse and syntactic analysis; classic quantitative work treats the location of possible switches as an empirical question rather than a self-evident denominator ([Pfaff 1975](https://eric.ed.gov/?id=ED127828)).

If the six indicators are [independent](../foundations/independence.qmd) and share one switch probability $\pi$, then $K$ has a [**binomial distribution**](https://bruno.nicenboim.me/bayescogsci/ch-intro.html):
...
Because the six indicators are independent, each qualifying sequence has the same product probability $\pi^2(1-\pi)^4$. Because the 15 sequences are mutually exclusive, those equal probabilities add:
