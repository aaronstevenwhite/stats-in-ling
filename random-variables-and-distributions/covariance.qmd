...
The [variance page](variance-and-covariance.qmd) averaged a squared deviation for one variable. Covariance instead averages the product of paired deviations from a [joint distribution](joint-probability.qmd). The linguistic question is whether less predictable words tend to receive longer reading times, an association studied directly in [work on predictability and reading time](https://doi.org/10.1016/j.cognition.2013.02.013). Suppose a constructed probability model places equal mass on four word tokens with contextual surprisal $X$ and reading time $Y$.
...
Covariance asks whether the two variables depart from their means in the same direction. In this example, do tokens with above-average surprisal also tend to have above-average reading times? The constructed association illustrates the summary; it does not by itself establish why the reading-time difference arises.
...
This unit dependence makes raw covariance difficult to compare across pairs measured on different scales. [Correlation](correlation.qmd) standardizes the quantity.
