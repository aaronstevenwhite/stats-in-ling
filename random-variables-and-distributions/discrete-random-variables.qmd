...
The [formal definition of a random variable](defining-random-variables.qmd) requires a measurable mapping from outcomes to values. We now ask a different question: can the possible values be listed one at a time? The answer separates discrete variables from the continuous variables introduced later.

Suppose $L$ maps each utterance outcome to its number of word tokens. We declare the
codomain $\operatorname{cod}(L)=\mathbb Z_{\geq0}$, whose values begin

$$
0,1,2,3,\ldots
$$

There may be no fixed largest utterance length in the model, but the codomain values
can be placed in a list. This property makes $L$ a [**discrete random variable**](https://bruno.nicenboim.me/bayescogsci/ch-intro.html).

Here *word token* means a unit produced by a declared tokenization scheme. Word segmentation is not identical across languages or annotation projects, so a real analysis must document the rule used to obtain the count (see the [Universal Dependencies tokenization guidelines](https://universaldependencies.org/u/overview/tokenization.html)).

The codomain declares which values the function may return. The
[**support**](https://bruno.nicenboim.me/bayescogsci/ch-intro.html) is the subset that
receives positive probability:

$$
\operatorname{supp}(X)
\equiv\{x\in X:\mathbb{P}(X=x)>0\}.
$$

Here $x\in X$ abbreviates $x\in\operatorname{cod}(X)$, following the [notation
introduced on the preceding page](defining-random-variables.qmd#distinguishing-the-two-spaces).
For the utterance-length model, the support might equal the full codomain, but it need
not. A model could assign zero probability to some declared codomain values.

More specifically, a random variable $X$ is discrete when some finite or countably infinite set $\mathcal S$ satisfies $\mathbb{P}(X\in\mathcal S)=1$. In this module, we declare a countable codomain for a discrete variable and use $x\in X$ when summing over it. *Countably infinite* means that every value can be assigned a position in a sequence, even though the sequence has no final value.
...
Thus discreteness concerns whether the probability-carrying values are separate and
listable. It does not determine whether the codomain is ordered or whether arithmetic
on its labels is meaningful.
...
The codomain declares the values to which a PMF assigns mass, including zero mass. The
[next page](probability-mass-functions.qmd) defines that assignment.
