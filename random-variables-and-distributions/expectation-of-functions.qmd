...
The [preceding page](expected-values.qmd) defined expectation as a probability weighted average. Now suppose the scientific quantity is not word length itself but a cost computed from word length. Should we average first and then transform, or transform each possible value and then average? This page separates those two operations.

Let $L$ record the length of a word in phonological units, with this constructed PMF.
...
## Transforming first, then averaging
...
For a discrete random variable, when
$\sum_{x\in\operatorname{supp}(X)}|g(x)|p_X(x)<\infty$,

$$
\mathbb{E}[g(X)]
\equiv\sum_{x\in\operatorname{supp}(X)}g(x)p_X(x).
$$
...
$$
\mathbb{E}[g(X)]
\equiv\int_{-\infty}^{\infty}g(x)f_X(x)\,\mathrm{d}x.
$$
