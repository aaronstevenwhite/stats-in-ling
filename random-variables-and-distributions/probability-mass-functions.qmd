...
The [preceding page](discrete-random-variables.qmd) used support to list the possible values of a discrete random variable. Suppose $S$ records the number of syllables in a sampled word token. Knowing that $S$ is discrete does not tell us how likely its support values are.
...
The function assigning a probability to each support value is a [**probability mass function**](https://bruno.nicenboim.me/bayescogsci/ch-intro.html), abbreviated [**PMF**](https://bruno.nicenboim.me/bayescogsci/ch-intro.html). We write
...
$$
p_S(2)=\mathbb{P}(S=2)=.33.
$$

This statement assigns $.33$ of the total probability to the event containing two syllable word outcomes. In this constructed model, $\operatorname{supp}(S)=\{1,2,3,4\}$.

## Check the two PMF requirements

First, a PMF must assign nonnegative mass:

$$
p_S(s)\geq0
\qquad\text{for every support value }s.
$$
...
## Add mass across selected values
