...
The [preceding page](discrete-random-variables.qmd) used support to list the possible values of a discrete random variable. Suppose $S$ records the number of syllables in a sampled word token. Knowing that $S$ is discrete does not tell us how likely its support values are. What object pairs each possible count with its probability?
...
Basically, we need a lookup rule from values to probabilities. More specifically, the function assigning a probability to each support value is a [**probability mass function**](https://bruno.nicenboim.me/bayescogsci/ch-intro.html), abbreviated [**PMF**](https://bruno.nicenboim.me/bayescogsci/ch-intro.html). We write
...
$$
\sum_{s\in\operatorname{supp}(S)}p_S(s)=1.
$$
