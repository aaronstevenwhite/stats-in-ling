...
The [expected value page](expected-values.qmd) summarized the center of a distribution. But can the mean distinguish a response that is nearly constant from one that often lies far from its center? Suppose $D$ records the dependency length of a selected syntactic relation. Two probability models may have the same mean dependency length while differing in how tightly their mass is concentrated around that mean.

In corpus work, dependency length is computed from a declared syntactic representation, commonly as the word distance between linked elements; cross-linguistic work compares observed lengths with representation-specific baselines ([Futrell, Mahowald, and Gibson 2015](https://doi.org/10.1073/PNAS.1502134112)). The toy PMFs below illustrate spread and do not constitute evidence for a processing explanation.
...
Basically, we need to average distance from the mean without letting positive and negative deviations cancel. The [**variance**](https://bruno.nicenboim.me/bayescogsci/ch-intro.html) does this by summarizing expected squared distance from the mean:
