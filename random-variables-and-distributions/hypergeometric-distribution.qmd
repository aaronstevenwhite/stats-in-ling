...
The [binomial model](binomial-distribution.qmd) used independent draws with a shared target probability. Suppose instead that an annotation archive contains 30 distinct clauses, nine passive and 21 active, and we choose five distinct clauses without replacement. What changes when selecting one clause changes which clauses remain available?
...
The archive counts are constructed, and *passive* stands for the output of a declared syntactic annotation scheme. The distribution models sampling those annotations; it does not turn an annotation into theory-neutral ground truth.
...
Basically, we must count samples from the finite archive rather than multiply one unchanged target probability across independent draws. More specifically, this count has a [**hypergeometric distribution**](https://openstax.org/books/introductory-statistics-2e/pages/4-5-hypergeometric-distribution).
