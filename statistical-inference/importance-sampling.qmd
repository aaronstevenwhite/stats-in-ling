...
We want the posterior probability that $\Theta>.70$, but suppose direct target draws are unavailable. The question is whether draws from another distribution can still recover a target expectation. We can draw from a proposal density $q(\theta)$ and correct the mismatch between proposal and target with weights.
