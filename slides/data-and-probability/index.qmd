...
## Use a corpus we can inspect

George Washington's 1793 inaugural address is included with `quanteda`.

::: {.incremental}
- 135 alphabetic word tokens after lowercasing and punctuation removal
- 90 word types
- Short enough to inspect, but large enough to have repeated words
:::

<span class="source">quanteda, `data_corpus_inaugural`</span>

::: {.notes}
[Sources]

https://quanteda.io/reference/data_corpus_inaugural.html
:::

## Count the tokens in R {.smaller}
...
word <- strsplit(
  trimws(normalized), "[[:space:]]+"
)[[1]]
frequency <- sort(table(word), decreasing = TRUE)
```

## Rank the observed types {.small}
